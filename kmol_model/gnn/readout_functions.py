...
class AttentionReadOut(GlobalAttention):
    def __init__(self, in_channels: int, full: bool = True, **kwargs):
        """
...
class CombinedReadOut(torch.nn.Module):
    def __init__(self, read_out_list: Tuple[str, ...], read_out_kwargs: dict):
        super().__init__()
...
def get_read_out(read_out: Union[str, Tuple[str, ...]], read_out_kwargs: Dict):
